...
namespace AsciiDocNet
{
    public class TableParser : ProcessBufferParserBase
    {
        ...
        {
            var isMatch= PatternMatcher.Table.IsMatch(reader.Line);
        ...
        public override void InternalParse(
            Container container,
            ...
            ref AttributeList attributes)
        {
            ...
            var table = new Table();
            while (reader.Line != null && !PatternMatcher.Table.IsMatch(reader.Line))
            {
                var columns = reader.Line.Split(',');

                table.Rows.Add(new Row(columns));
            }
